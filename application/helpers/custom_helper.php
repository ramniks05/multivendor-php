...
//get logo name
if (!function_exists('get_logo_name')) {
    function get_logo_name()
    {
        $ci =& get_instance();
        $name = '';
        if (!empty($ci->general_settings->application_name)) {
            $name = $ci->general_settings->application_name;
        } elseif (!empty($ci->settings->application_name)) {
            $name = $ci->settings->application_name;
        } elseif (!empty($ci->settings->site_title)) {
            $name = $ci->settings->site_title;
        }
...
if (!function_exists('get_brand')) {
    ...
    {
        $ci =& get_instance();
        if (empty($id) || empty($ci->brand_model)) {
            return null;
        }
        return $ci->brand_model->get_brand($id);
    }
}
...
if (!function_exists('get_brand_name')) {
    function get_brand_name($brand)
    {
        ...
        $ci =& get_instance();
        return parse_serialized_name_array($brand->name_data, $ci->selected_lang->id);
    }
}
